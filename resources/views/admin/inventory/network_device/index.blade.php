...
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
    ...
    <div class="d-flex align-items-center flex-wrap text-nowrap">   
        <a href="{{route('network-devices.create')}}" class="btn btn-info btn-icon-text mb-2 mb-md-0">
...
<div class="row">
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">Data Network Device</h6>
                        ...
                        <tbody>
                            @foreach ($datas as $data)
                                <tr>
                                    ...
                                    <td>{{ $data->getVendor->name ?? 'Aset PLN' }}</td>
                                    <td>{{ $data->year }}</td>
                                    <td>
                                        <a href="{{ route('network-devices.edit', ['network_device' => $data]) }}" class="btn btn-sm btn-primary btn-icon">
                                            <i data-feather="edit"></i>
                                        </a>
                                        <form action="{{ route('network-devices.destroy', ['network_device'=> $data]) }}" style='display:inline;'  method="POST"  class="me-2" style="cursor:pointer;">
                                            @method('DELETE')
                                            @csrf
